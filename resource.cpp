...
void Game::loadWGP(const char *fileName) {
	...
	if (tag == 0x4D42) { // _10.SCN (uncompressed .bmp)
		...
		do {
			const int sz = fp->readUint16_tLE();
			if (fp->ioErr()) {
				break;
			} else if (sz != 0) {
...
void Game::loadSPR(const char *fileName, SceneAnimation *sa) {
	...
	while (1) {
		...
		for (int i = 0; i < num; ++i) {
			...
			frame->hdr.num = fp->readUint16_tLE();
			frame->hdr.w = fp->readUint16_tLE();
			frame->hdr.h = fp->readUint16_tLE();
			frame->hdr.xPos = fp->readUint16_tLE();
			frame->hdr.yPos = fp->readUint16_tLE();
...
void Game::loadMOV(const char *fileName) {
	...
	char wgpName[128];
	len = fp->readUint16_tLE();
	fp->read(wgpName, len);

	int speed = fp->readUint16_tLE(); // anim speed in ms
	...
	while (1) {
		...
		case 2:
			while (1) {
				...
				box->x1 = fp->readUint16_tLE();
				box->y1 = fp->readUint16_tLE();
				box->x2 = fp->readUint16_tLE();
				box->y2 = fp->readUint16_tLE();
		...
		case 3: {
				...
				while (1) {
					int initType = fp->readUint16_tLE();
					if (initType == 0) {
						break;
					}
					switch (initType) {
					case 2000:
						...
						break;
					case 3000:
						so->mode = fp->readUint16_tLE();
						if (so->mode == 2) {
							so->modeRndMul = fp->readUint16_tLE();
						}
						break;
					case 3500:
						so->xInit = fp->readUint16_tLE();
						so->yInit = fp->readUint16_tLE();
						break;
					case 4000:
						so->zInit = fp->readUint16_tLE();
						break;
					case 4500:
						so->flipInit = fp->readUint16_tLE();
						break;
					case 5000:
						so->motionFrameNum = fp->readUint16_tLE();
						--so->motionFrameNum;
						break;
					case 5500:
						so->motionNum = fp->readUint16_tLE();
						--so->motionNum;
						break;
					case 6000: {
							int var = fp->readUint16_tLE();
							assert(var >= 0 && var < 10);
							so->varsTable[var] = fp->readUint16_tLE();
						}
						break;
					default:
						assert(0);
			...
			break;
		case 4:
			sa->scriptSize = fp->readUint16_tLE();
			if (sa->scriptSize != 0) {
